...
| 2026-09-06 | **Added the ACI-IoT-2023 data artifact as a separate bib entry, so the dataset's paper and its data are cited distinctly.** `ref:aciiot` remains the MILCOM 2024 paper describing the dataset; the new `ref:aciiotdata` is the IEEE DataPort deposit, DOI `10.21227/qacj-3x32`, confirmed against `api.datacite.org` (DataPort registers with DataCite, not Crossref, which is why the earlier Crossref pass did not surface it). The two differ in more than the DOI: the deposit lists Bastian, Bierbrauer, McKenzie and Nack, while the paper lists Nack, McKenzie and Bastian, so they are genuinely different records and each is labelled inline as THE PAPER or THE DATA ARTIFACT. A matching DataCite search found no DOI for the CIC-IoT-2023 data artifact — only third-party work using it — so CIC keeps a single entry, and that negative is recorded in the file rather than left to be re-derived. Note that `ref:aciiotdata` is currently uncited, so BibTeX will omit it from the rendered bibliography until a `\cite` or `\nocite` refers to it; whether to cite the data alongside the paper in the Methodology is left open. |
| 2026-09-06 | **Cited the ACI-IoT-2023 data artifact alongside its paper in the Methodology, so every bib entry now renders.** The sentence introducing the two benchmarks in Section "Datasets and Preprocessing" now reads `\cite{ref:aciiot,ref:aciiotdata}`, pairing the MILCOM 2024 paper that describes the dataset with the IEEE DataPort deposit of the data itself; the line was rewrapped to the file's 80-column style, which is the only other change to the prose. This was the one substantive content edit in an otherwise format-and-metadata sequence, and it was made deliberately rather than as a side effect: BibTeX omits uncited entries, so adding `ref:aciiotdata` to the bibliography in the previous commit would have had no visible effect until something cited it. All 15 entries are now cited and all 15 resolve. CIC-IoT-2023 is still cited once, since its data carries no DOI of its own. |
| 2026-09-06 | **Drafted the three Elsevier submission elements Computers & Security requires beyond the manuscript body: Highlights, a CRediT statement and a Data Availability statement.** `paper/highlights.txt` is a new standalone file (Elsevier wants Highlights uploaded separately, not inside `main.tex`) holding five bullets — the four abstract findings plus the unifying thesis — each 68-77 characters against the journal's 85-character limit. The CRediT and Data Availability sections were inserted into `main.tex` as `\section*` blocks between the Conclusion and the bibliography, which is where elsarticle expects them. **The CRediT role split is a documented placeholder, not a finding:** PROGRESS.md records the research decisions but nothing about the division of labour between the two authors, so no split was inferred — the first author was given the implementation and drafting roles and the second the supervisory and review roles, under an explicit `TODO(author)` telling the corresponding author to confirm before submission. The Data Availability statement cites `ref:aciiotdata` (the IEEE DataPort deposit) and `ref:ciciot` for the two benchmarks, points at `scripts/` and `experiments/` for the code and run records, and repeats that the injection study's telemetry field is simulated. It carries no repository URL: this repo has no git remote configured, so no public location exists yet and none was invented, per the project's policy of never guessing citation metadata. A Zenodo or figshare DOI snapshot is noted in the TODO as preferable to a bare repository URL for a reproduction paper. Nothing was compile-verified — no LaTeX toolchain is installed on this machine, so the two new sections have not been rendered. |
| 2026-09-06 | **Published the repository to GitHub, then removed `paper/` from tracking so the code repo and the manuscript live apart.** The repo was pushed to `https://github.com/reoroman14/ids-agent-repro.git` (private) as branch `main`, renamed from `master`; 10 commits, 118 files, 2.2 MB. Before pushing, the tracked tree was scanned for credentials and the only matches were environment-variable *names* plus a deliberate fake `sk-secret` in `tests/verify_agent.py`, which exists to assert that a key placed in a config file is ignored — no real secret is tracked, and `data/raw|processed|splits`, `.venv/`, `.idea/` and `paper/*.pdf` were already excluded. Immediately afterwards `paper/` was untracked with `git rm -r --cached` and `.gitignore` was changed from ignoring only LaTeX build artifacts to ignoring the whole directory, on the decision that the manuscript source belongs on Overleaf and on disk while this repo carries only code, configuration and experiment records. All four files in `paper/` (including the drafts made earlier today) are untouched on disk. **This does not erase the manuscript from GitHub:** `main.tex`, `references.bib` and `highlights.txt` remain in the history of the commits made before this one and can still be retrieved by SHA (resolved later the same day by the history purge recorded in the next entry). Removing them from history would need a rewrite and a force-push, which was flagged and deliberately not done. A consequence worth remembering: the paper source now has no version-control history going forward, so Overleaf's own revision history is the only record of its changes. |
| 2026-09-06 | **Purged `paper/` from the entire commit history and force-pushed, so the manuscript is no longer retrievable from any commit.** Untracking it earlier removed it from the current tree only; the source remained fetchable by SHA from the old commits, which is why this second step was requested. `git filter-branch --index-filter 'git rm -r --cached --ignore-unmatch paper/' --prune-empty` was run over all refs, then `refs/original/` was deleted, the reflog expired and `git gc --prune=now` run, and the result force-pushed with `--force-with-lease`. **The purge covered the whole `paper/` path, not just the three files named in the request:** history also held `abstract.tex`, `intro_related.tex`, `methodology.tex`, `results.tex`, `discussion.tex` and `conclusion.tex`, the section files merged into `main.tex` by an earlier commit, so purging only `main.tex`, `references.bib` and `highlights.txt` would have left the paper's full text in history under the older filenames. Every SHA from the initial commit onward changed; the commit count went from 11 to 10 because `paper: set author block and running head` touched nothing but `paper/` and was pruned as empty. Commit *messages* were not rewritten, so subjects like "paper: audit DOIs in references.bib" still describe manuscript work even though no manuscript content survives. A full pre-rewrite bundle and copies of all four `paper/` files were taken to the session scratchpad first, and the four files on disk were verified untouched afterwards. **Known limitation, flagged before the rewrite and confirmed after:** a force-push does not make GitHub delete the old objects immediately — they become unreachable and are removed only when GitHub's own garbage collection runs, so old commit SHAs may resolve on the web for a period. Deleting and recreating the repository is the only way to guarantee immediate removal. |
| 2026-09-06 | **Deleted and recreated the GitHub repository, then pushed the purged history; the manuscript is now unreachable from GitHub by any route.** The force-push alone had not achieved this: the old commits survived as unreachable objects and both the pre-rewrite tip commit and its parent were still fetchable by SHA, and `main.tex` and `highlights.txt` could be read straight back out of them. That was verified empirically rather than assumed, and it is why the repository was destroyed and rebuilt rather than merely force-pushed. Against the recreated repo the push reported `* [new branch]`, confirming the remote was genuinely empty. Verification from a fresh clone, not the working directory: zero `paper/` paths across all commits, zero `.tex` or `.bib` objects in the whole object database, and fetching the pre-rewrite tip commit by SHA now fails with `upload-pack: not our ref`. Commit messages were also rewritten to drop the dead SHA, since a `filter-branch --index-filter` rewrites trees but leaves messages untouched. **One residue is known and accepted:** the pre-rewrite tip SHA still appears as a literal string inside the `PROGRESS.md` and `.gitignore` *content* of two ancestor commits, which recorded the then-true statement that the paper remained in history. It is a dangling hex string naming an object that no longer exists in any repository, so it leaks no manuscript content; scrubbing it would need a third rewrite and was not judged worth it. |
| 2026-09-06 | **Third and final rewrite: scrubbed the dead pre-rewrite SHAs out of file *contents* as well, so no trace of the purged commits survives anywhere in the repository.** The earlier `--index-filter` purge rewrote trees and a `--msg-filter` pass rewrote commit messages, but three ancestor commits still carried the old tip SHA as a literal string inside their `PROGRESS.md` and `.gitignore` text — notes that were true when written, recording that the manuscript was still in history. A `--tree-filter` pass replaced those hex strings with prose across all refs. The rewrite is provably confined to ancestors: the HEAD tree hash was `776be819cd261dd27092406880f1ce1eb13e3908` both before and after, byte-identical, because HEAD had already been cleaned by hand. Verified from a fresh clone that no dead SHA appears in tracked content at any commit, none appears in any commit message, no `paper/` path exists in any commit, and the object database holds zero `.tex` or `.bib` objects. A residual caveat, same in kind as the one that forced the repository to be recreated earlier: this was a force-push rather than another delete-and-recreate, so the pre-scrub commits linger on GitHub as unreachable objects until its garbage collection runs. What those objects contain is only the obsolete hex strings in a log file, not manuscript text, so recreating the repository a second time was judged unnecessary. Note for future sessions: describe commits in prose in `PROGRESS.md` rather than by SHA, since any rewrite turns recorded SHAs into dangling references that then need scrubbing themselves. |
| 2026-09-06 | **Filled the repository URL into the paper's Data Availability statement, replacing the "no URL exists" placeholder.** The statement now reads "available at \url{https://github.com/reoroman14/ids-agent-repro}" and continues to point at `scripts/` for the experiment scripts and `experiments/` for the run records, to cite the two benchmarks as third-party public datasets, and to repeat that the prompt-injection telemetry field is simulated. `\url` is provided by `hyperref`, already loaded in the preamble — but the preamble comment invites removing `hyperref` if the venue objects, which would now break this line, so `url` would have to be loaded in its place. **The TODO was replaced rather than deleted, because a new problem takes its place: the repository is private, so the printed link 404s for any reviewer or reader who follows it.** It must be made public before submission, and a Zenodo or figshare DOI snapshot remains preferable to a bare GitHub URL for a reproduction paper, since GitHub URLs are not archival. Note that `paper/` is no longer tracked in git, so this edit exists only on disk and in Overleaf if synced by hand; the manuscript's change history now lives solely in Overleaf. |
| 2026-09-07 | **Removed every em dash from the manuscript's prose, replacing each with standard punctuation chosen from context.** The source contained no literal U+2014 characters; all em dashes were LaTeX `---`. A raw count of 156 was misleading, since most hits were the `% --- section ---` comment rules: the true figure was 67 in non-comment text, of which 4 sit inside tables (the "not applicable" cells in the injection table and the `Stage A ---` / `Stage B ---` row headers in the poisoning table) and were deliberately left, per the instruction not to touch table content. That left **63 prose dashes, fixed in 38 edits** (a matched pair counting as one edit), applied as 40 string replacements by a script that aborts without writing if any pattern is missing or non-unique. By resulting punctuation: 19 became parentheses, 9 commas, 4 colons, 3 semicolons, and 3 were split into separate sentences, with one case taking a colon plus a sentence split. **No dash was kept**, as every case had a natural alternative. Four edits change more than punctuation and were flagged to the user as such before being applied: three sentence splits reword slightly (`not a macro one. The distinction materially changes...`, `on a matched sample. That is a ceiling, not a result.`, `surface form. The same tool-deference that limits...`), and one adds the word "by" twice to keep a list of examples parallel. Correctness checks after the edit: `---` in non-comment text fell from 67 to exactly the 4 table cells, brace balance was unchanged at 308/308, and parenthesis count rose from 114 to 133, exactly the +19 predicted by the 19 parenthesis conversions. Two lines that grew past the file's ~80-column hand wrapping were rewrapped; the only remaining over-long lines predate this change. The user reviewed all 38 proposed edits with before/after context before any were applied, which is the standing expectation for manuscript prose. |
| 2026-09-07 | **Added `ref:adaptiveinjection` (Zhan, Fang, Panchal and Kang, Findings of NAACL 2025) and cited it in the prompt-injection subsection of Related Work.** The user supplied full metadata including the DOI, and, following the practice already established for the two dataset citations, it was independently resolved against `api.crossref.org` rather than taken on trust: title, all four authors, venue, year and DOI matched exactly. **The page range did not.** Crossref reports 7101--7117 while the user gave 7116--7132; the ACL Anthology, the publisher of record, gives 7116--7132, so the Anthology figures were used and the disagreement is documented inline in the entry as a stale Crossref deposit. No value was guessed. The citation frames the paper as evidence that injection defences fail under adaptive attack, which the sentence then connects to this study's own result: fencing prevented syntactic escape in every trial but not the semantic authority confusion that let a forged tool observation succeed. **One judgement call to note:** `references.bib` declared a venue policy of peer-reviewed journals plus, by flagged exception, top-tier security proceedings (USENIX Security). Findings of NAACL is peer-reviewed, through ACL Rolling Review, but it is neither a journal nor a security venue, so the header's venue-policy note was widened to name peer-reviewed ACL-family proceedings explicitly rather than leaving the header making a claim the file no longer satisfied. The entry and DOI counts in both the bib header and the citation-status comment in `main.tex` were updated from 15/11 to 16/12. Verified afterwards that all 16 keys are cited and all 16 resolve, with no orphans in either direction. Nothing was added from any preprint. |
| 2026-09-07 | **Removed the CRediT authorship contribution statement from `main.tex`; Elsevier collects it through Editorial Manager instead.** The corresponding author selects each co-author's roles from dropdowns at submission, so the statement is not manuscript text and would have been duplicated or contradicted by the submission record. The whole block went: the section heading, both author role lines, and the `TODO(author)` comment flagging that the split was a placeholder needing confirmation. **That TODO is now discharged rather than lost:** the roles still have to be chosen, but in Editorial Manager, not here. The Data Availability statement was deliberately left untouched, since that one does belong in the body, and it still sits between the Conclusion and the bibliography. Verified that no trace of the section survives (no occurrence of CRediT, Conceptualization, Supervision or the role-list formatting), that the junction left exactly one blank line with no stray whitespace, and that the counts reconcile exactly: braces 309 to 307 (three pairs removed with the heading and two `\textbf` role lines, one added by the header-comment update) and parentheses 133 to 132 (the pair inside `TODO(author)`). The document-order map in the file header was also extended to name `\section*{Data availability}`, which it had never listed. |
| 2026-09-07 | **Added the Elsevier generative-AI declaration to `main.tex`, between Data availability and the bibliography.** The wording was supplied by the author and inserted verbatim; a check confirmed the whitespace-normalised section text is character-identical to what was supplied, and an inline comment tells future editors not to paraphrase it. The statement records that Claude was used as a coding and data-analysis assistant for implementing, running and debugging the experiments under author supervision, and that it was not used to generate manuscript prose. The same request also asked again for the CRediT section to be removed, which had already been done in the previous change; it was verified absent rather than removed twice. Final structure confirmed as Introduction, Related Work, Methodology, Results, Discussion, Conclusion, Data availability, Declaration of Generative AI, bibliography. Cross-reference integrity checked after both edits: 31 labels defined and 25 referenced with **no reference to an undefined label**, so nothing broke when the CRediT section went. Six labels are defined without being referenced from `main.tex`, but two of those, `sec:intro` and `sec:related`, are referenced from note fields in `references.bib`, and the rest are ordinary unreferenced section anchors. All 16 cite keys resolve, braces and parentheses balance at 309 and 135. |
| 2026-09-07 | **Wrapped the last two over-long source lines in `main.tex`, after a rewrap script overran its range and had to be repaired.** The two targets were a 107-character line carrying the start of the IDS-Agent footnote in the Introduction and an 84-character line in the Data Availability statement. **The first attempt was wrong and is worth recording:** the paragraph range given to the greedy re-wrapper ran past the end of the Data Availability paragraph, so it absorbed the blank line and the following comment block and merged a `%` comment into the middle of the prose, leaving `Section~\ref{sec:methodology}. % --- generative AI declaration` on one line. This was caught immediately by inspecting the result rather than trusting the script, and the block was rebuilt from known-good text. The re-wrapper was then rewritten to stop at the first blank or comment line, so the range cannot silently swallow a neighbouring block, and to assert that the whitespace-normalised token stream is unchanged before writing. Verified afterwards that the manuscript prose is word-for-word identical at 6,623 words and the comment text at 275 words against the pre-declaration snapshot, that no label reference or cite key dangles, and that braces and parentheses still balance at 309 and 135. Some raggedness remains and is unavoidable: `\url{...}` and `\cite{...}` are single unbreakable tokens, so the line before a long one is sometimes short. Five lines of 81 to 82 characters elsewhere in the file were checked against the session's earliest snapshot, confirmed pre-existing and left alone, since all five are dense mathematics or citation-heavy lines where re-wrapping buys nothing. **Standing lesson for this file: give a re-wrapper an explicit paragraph boundary, never an approximate line range.** |
---
