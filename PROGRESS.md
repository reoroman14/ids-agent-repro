...
| 2026-09-06 | **Deleted and recreated the GitHub repository, then pushed the purged history; the manuscript is now unreachable from GitHub by any route.** The force-push alone had not achieved this: the old commits survived as unreachable objects and both an earlier commit and its parent commit were still fetchable by SHA, and `main.tex` and `highlights.txt` could be read straight back out of them. That was verified empirically rather than assumed, and it is why the repository was destroyed and rebuilt rather than merely force-pushed. Against the recreated repo the push reported `* [new branch]`, confirming the remote was genuinely empty. Verification from a fresh clone, not the working directory: zero `paper/` paths across all commits, zero `.tex` or `.bib` objects in the whole object database, and `git fetch origin an earlier commit...` now fails with `upload-pack: not our ref`. Commit messages were also rewritten to drop the dead SHA, since a `filter-branch --index-filter` rewrites trees but leaves messages untouched. **One residue is known and accepted:** the string an earlier commit still appears inside the `PROGRESS.md` and `.gitignore` *content* of two ancestor commits, which recorded the then-true statement that the paper remained in history. It is a dangling hex string naming an object that no longer exists in any repository, so it leaks no manuscript content; scrubbing it would need a third rewrite and was not judged worth it. |
---
